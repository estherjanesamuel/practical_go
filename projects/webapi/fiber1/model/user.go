...
func (u User) Migrate () (err error){
	...
	_, err = u.db.Exec(`
	ALTER TABLE users ADD column name VARCHAR(64) NOT NULL DEFAULT ''
	`)
...
func (u *User) FindByEmail(email string) (found bool) {
	err := u.db.Get(u, `SELECT id, email, password, name FROM users WHERE email = ?`, email)
	if err != nil {
...
func (u *User) CheckPassword(password string) bool {
	...
	if err != nil {
		log.Println(`User.CheckPassword: %v`, err)
		return false
	}
	return true
}
...
func (u *User) Insert() bool {
	ra, err := u.db.Exec(`INSERT INTO users (email, password, name)	VALUES (?, ?, ?)`, u.Email, u.Password, u.Name)
	if err != nil {
	...
	if err != nil {
		log.Printf(`User.LasInsertId: %v`, err)
	}
	u.Id = id
	return true
}
